    ...
    public Map<String, Map<String, List<SlotsDTO>>> fivedays(int sid, String date) throws ResourceNotFoundException {
        ...
        for(int i=0;i<5;i++) {
            ...
            if(holidaysRepository.existsByDate(date)){
                SlotsDTO slotsDTO =new SlotsDTO(null,null);
                Map<String, List<SlotsDTO>> mp=new LinkedHashMap<>();
                List<SlotsDTO> slotsDTOArrayList=new ArrayList<SlotsDTO>();
                //slotsDTOArrayList.add(slotsDTO);
                mp.put("Holiday",slotsDTOArrayList);
               map.put(date  ,mp);//+"is a Holiday"
                log.info("holiday"+date);
